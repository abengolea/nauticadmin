 ...
 * medidos contra las medianas de la escuela, para identificar rápidamente áreas de mejora y adaptar los planes de entrenamiento de manera efectiva.
 *
 * - analyzePlayerPerformance - Una función que analiza los datos de evaluación física de un jugador en comparación con las medianas de la escuela.
...
const AnalyzePlayerPerformanceInputSchema = z.object({
  ...
  escuelaId: z.string().describe('El ID de la escuela a la que pertenece el jugador.'),
...
const AnalyzePlayerPerformanceOutputSchema = z.object({
  analysis: z.string().describe('Un análisis detallado de las tendencias de rendimiento del jugador en comparación con las medianas de la escuela, destacando áreas de mejora.'),
});
...
Se te proporcionan los datos de rendimiento de un jugador dentro de su escuela para un tipo de prueba específico.
Analiza la tendencia de rendimiento del jugador a lo largo del tiempo en comparación con el rendimiento medio de la escuela en la misma prueba.
Identifica las fortalezas y debilidades del jugador y sugiere áreas de mejora.

ID del Jugador: {{{playerId}}}
ID de la Escuela: {{{escuelaId}}}
