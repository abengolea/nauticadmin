'use server';
/**
 * @fileOverview Flujo de Genkit para analizar y comparar evaluaciones de clientes.
 *
 * - generateComparativeAnalysis: Genera un análisis comparativo de un cliente.
...
const GenerateComparativeAnalysisInputSchema = z.object({
  playerName: z.string().describe('Nombre del cliente a analizar.'),
  playerData: PlayerEvalDataSchema.describe('Datos de la evaluación más reciente del cliente.'),
  ...
  comparisonContext: z.string().describe('Contexto de los datos de comparación (ej. "el promedio de su categoría", "un cliente de élite", etc.).')
});
...
const analysisPrompt = ai.definePrompt({
  ...
  prompt: `
    Actúa como un asistente de análisis de NauticAdmin.
    Tu tarea es redactar un análisis comparativo CONSTRUCTIVO y FORMATIVO para el cliente {{playerName}}.
    ...
    **1. Resumen General:**
    Un párrafo introductorio que resuma el perfil del cliente basado en la data.

    **2. Fortalezas Destacadas:**
    Una lista (usando viñetas) de 2-3 puntos donde el cliente destaca en comparación con la referencia. Sé específico y usa los datos.
    ...
    **4. Conclusión y Próximos Pasos:**
    Un párrafo final que ofrezca ánimo y sugiera un enfoque para los próximas actividades.

    Utiliza un tono profesional, alentador y alineado con los valores formativos de NauticAdmin.
    NO uses jerga excesivamente técnica. El informe puede ser leído por operadores y, eventualmente, por clientes.

    **Datos del Cliente ({{playerName}}):**
